<?php 
    ...
    if(isset($_GET['like']))
    {
        ...
        $queryLike = "SELECT * FROM codregistros WHERE idcr LIKE '%$valor%' OR nombre_registro LIKE '%$valor%'";

        $datareg = CRUD($queryLike,"s");
    }
    else{
        $datareg = CRUD("SELECT * FROM codregistros ORDER BY idcr LIMIT $inicio,$registros", "s");
...
?>
    ...
    <div style="margin-bottom: 10px;">
        <div class="row">
            ...
            <div class="col-md-6">
                <input type="search" class="form-control" placeholder="Busca Registro" id="like-reg" autocomplete="off">
            </div>
        </div>
    </div>
    <?php if($datareg):?>
        ...
        <?php if($num_registro > $registros):?>
            <?php if($pagina == 1):?>
                <div style="text-align: center;">
                    <a href="" class="btn reg" v-num="<?php echo ($pagina + 1); ?>" num-reg="<?php echo $registros;?>">
